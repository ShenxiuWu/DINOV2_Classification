...
class MNISTFashionDataLoader:
    ...
    def _load_data(self):
        dataset = datasets.FashionMNIST(self.data_dir, download=True, train=True, transform=self.transform)
        train_size = int(self.train_split * len(dataset))
        val_size = len(dataset) - train_size
        self.train_dataset, self.val_dataset = random_split(dataset, [train_size, val_size])

        self.test_dataset = datasets.FashionMNIST(self.data_dir, download=True, train=False, transform=self.transform)
