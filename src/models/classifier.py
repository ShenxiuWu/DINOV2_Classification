...
class DinoVisionTransformerClassifier(nn.Module):
    ...
    def __init__(self, dino_model, embed_dim, num_classes,freeze_feature_extractor=False):
        ...
        new_proj = nn.Sequential(  # change the projectio
            nn.Conv2d(
                1,
                projtion_dim.out_channels,
                kernel_size=projtion_dim.kernel_size,
                stride=projtion_dim.stride,
                padding=projtion_dim.padding,
                bias=False,
            ),
            nn.Identity(),
        )
        if freeze_feature_extractor:
            print(f"Freezing the feature extractor")
            for param in self.dino_model.parameters():
                param.requires_grad = False
        print(f"Fine-tuning the feature extractor also")
